...
{
    public partial interface IOperableBitBoard<TSelf, TLineElement> : IBitBoard<TSelf, TLineElement>
        where TSelf : unmanaged, IOperableBitBoard<TSelf, TLineElement>
        where TLineElement : unmanaged, IBinaryNumber<TLineElement>, IUnsignedNumber<TLineElement>, IBinaryInteger<TLineElement>
    {
        ...
        static TSelf IBitBoard<TSelf, TLineElement>.Empty => TSelf.CreateFilled(TSelf.EmptyLine);
        static TSelf IBitBoard<TSelf, TLineElement>.AllBitsSet => ~TSelf.Zero;

        static TSelf IBitBoard<TSelf, TLineElement>.InvertedEmpty => ~TSelf.Empty;
        #endregion
        ...
        /// 
        /// </summary>
        /// <param name="x">The full-range x coordinate.</param>
        /// <param name="y"></param>
        /// <returns></returns>
        ...
        {
            for (var i = 0; i < TSelf.Height; i++)
            {
                var m = board[i];
                if (!TLineElement.IsZero(m))
                {
                    int j = 0;
                    var tz = TLineElement.TrailingZeroCount(m);
                    var s = int.CreateTruncating(tz);
                    while (tz < TLineElement.TrailingZeroCount(TLineElement.Zero))
                    {
                        m >>>= s + 1;
                        j += s;
                        yield return new Point(TSelf.StorableWidth - j - 1, i);
                        j++;
                        tz = TLineElement.TrailingZeroCount(m);
                        s = int.CreateTruncating(tz);
                    }
                }
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static virtual TSelf ShiftDown16Lines(TSelf board, TLineElement upperFeedValue) => TSelf.ShiftDownEightLines(TSelf.ShiftDownEightLines(board, upperFeedValue), upperFeedValue);
        ...
        static virtual TSelf ShiftDownVariableLines(TSelf board, int count, TLineElement upperFeedValue)
        {
            ...
            {
                c = d;
                b = TSelf.ShiftDown16Lines(b, upperFeedValue);
                d -= 16;
            }
            if ((c & 8) > 0)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static virtual TSelf ShiftUp16Lines(TSelf board, TLineElement lowerFeedValue) => TSelf.ShiftUpEightLines(TSelf.ShiftUpEightLines(board, lowerFeedValue), lowerFeedValue);
        ...
        static virtual TSelf ShiftUpVariableLines(TSelf board, int count, TLineElement lowerFeedValue)
        {
            ...
            {
                c = d;
                b = TSelf.ShiftUp16Lines(b, lowerFeedValue);
                d -= 16;
            }
            if ((c & 8) > 0)
            {
        ...
        static abstract int GetBlockHeight(TSelf board);
